...
export class TaskSheetComponent implements OnInit, OnDestroy {
  ...
  public constructor(
    @Inject(MAT_BOTTOM_SHEET_DATA) data: { selected: Task | null },
  ...
  public ngOnInit(): void {
    ...
    this.tasks$ = this.store.select(selectSingleMainTasks).pipe(map(tasks => tasks.filter(task => {
      if (this.selected === null) {
        return task.alarm === null;
      } else {
        return task.id === this.selected.id ? true : task.alarm === null;
      }
    })));
  ...
  public change(): void {
    this.bottomSheetRef.dismiss(this.taskInput.value.id);
  }
  ...
  private filter(value: string | Task): void {
    ...
    this.filteredTasks$ = this.tasks$.pipe(map(task => task.filter(task => task.name.toLowerCase()
